...
class _CustomLoaderState extends State<CustomLoader> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final loaderColor = widget.color ?? (isDark ? Colors.white : Colors.black87);
...
class _RadiantWreathPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final activeFillPaint = Paint()
      ..shader = sweepGradient.createShader(Rect.fromCircle(center: center, radius: size.width))
    ...
    final stemPath = Path()..addOval(Rect.fromCircle(center: center, radius: radius));
    ...
    for (int i = 0; i < segments; i++) {
      ...
      final base = Offset(center.dx + radius * math.cos(angle), center.dy + radius * math.sin(angle));
