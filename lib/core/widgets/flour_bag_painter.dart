...
class FlourBagPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const bagColor = Colors.white;
    const shadeColor = Color(0xFFDEDEDE);
    const knotColor = Color(0xFF777777);
    const inkColor = Colors.black87;
    ...
    const ropeColor = Color(0xFFC0A080); // Brownish rope color
    ...
    if (look == 2) {
      ...
      // Blush
      canvas.drawOval(
        Rect.fromCenter(center: Offset(cx - 27, cy + 1), width: 12, height: 7),
  ...
  void _drawCovering(
    ...
    if (drop > 5) {
      ...
      final sp = Paint()
        ...
        ..strokeCap = StrokeCap.round;
      canvas.drawArc(
        Rect.fromCenter(center: Offset(cx - 14, cy - 12), width: 16, height: 8),
        0,
        math.pi,
        false,
        sp,
    ...
    canvas.drawOval(
      Rect.fromCenter(center: Offset(cx - 28, cy + 4), width: 15, height: 9),
  ...
  void _drawHandPad(
    ...
    for (int i = 0; i < 4; i++) {
      ...
      canvas.drawCircle(
        Offset(fx + 1, fy + 1),
        5.2,
        Paint()..color = shade.withValues(alpha: 0.2),
      );
  ...
  void _drawEye(
    ...
    canvas.drawOval(
      Rect.fromCenter(center: Offset(x, y), width: 21, height: 16),
      Paint()..color = white,
    );
    canvas.drawOval(
      Rect.fromCenter(center: Offset(x, y), width: 21, height: 16),
    ...
    canvas.drawCircle(
      Offset(px, py - 2.2),
      1.7,
      Paint()..color = white.withValues(alpha: 0.82),
    );
  }
  ...
  void _drawNarrowEye(
    ...
    if (h < 1.0) return;
    canvas.drawOval(
      Rect.fromCenter(center: Offset(x, y), width: 21, height: h),
      Paint()..color = white,
    );
    canvas.drawOval(
      Rect.fromCenter(center: Offset(x, y), width: 21, height: h),
    ...
    canvas.drawCircle(
      Offset(x, y + h * 0.12 - pr * 0.35),
      (pr * 0.3).clamp(0.5, 2.0),
  ...
  void _drawBrowDot(Canvas canvas, double x, double y, Color color) {
    ...
    canvas.drawOval(
      Rect.fromCenter(center: Offset(x - 4, y), width: 5, height: 3.5),
      paint,
    );
    canvas.drawOval(
      Rect.fromCenter(center: Offset(x + 4, y), width: 5, height: 3.5),
